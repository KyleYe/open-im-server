...
func main() {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		// has api changes
		display := fmt.Sprintf("%s [\\#%s](https://github.com/KyleYe/open-im-server/pull/%s)", message, matches[1], matches[1])
...
func upstreamLinkify(line string) string {
	if m := upstreamKube.FindStringSubmatch(line); len(m) > 0 {
		return fmt.Sprintf("UPSTREAM: [#%s](https://github.com/KyleYe/open-im-server/pull/%s):%s", m[1], m[1], m[2])
	}
